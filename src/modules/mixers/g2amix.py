...
class G2AMixer(nn.Module):
    def __init__(self, scheme, args):
        super(G2AMixer, self).__init__()
        self.n_agents = args.n_agents
        self.scheme = scheme
        input_shape = self._get_input_shape()
        # Encoding
        self.encoding = nn.Linear(input_shape, args.rnn_hidden_dim)  # 对所有agent的obs编码
        ...
        # 输入的inputs维度为(n_agents - 1, batch_size * n_agents, rnn_hidden_dim * 2)，
    ...
    def forward(self, agent_qs, obs, hidden_state):   # 输入还需要加上当前时间点的individual_Qs（可能还需要state），输出Q_tot
        ...
        if self.args.hard:
            ...
            input_hard = input_hard.view(self.args.n_agents, -1, self.args.rnn_hidden_dim * 2)

            h_hard = th.zeros((2 * 1, size, self.args.rnn_hidden_dim))  # 因为是双向GRU，每个GRU只有一层，所以第一维是2 * 1
            ...
            hard_weights = hard_weights[:, 1].view(-1, self.args.n_agents, self.args.n_agents)
        ...
        scaled_score = score / np.sqrt(self.args.attention_dim)
            ...
            # softmax得到权重
        soft_weight = f.softmax(scaled_score, dim=-1).squeeze(1)  # (batch_size，1, n_agents - 1)
        ...
        # 合并每个agent的h与x
        # x = th.stack(x, dim=1).reshape(-1, self.n_agents)
